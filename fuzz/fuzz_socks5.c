 ...
 *
 * The request stage calls moor_socks5_parse_request() -- the parser split out
 * of the handler so it can be fuzzed without the circuit and hidden-service
 * routing behind it (a .moor CONNECT used to cost a thread per input). */
#include <stddef.h>
#include <stdint.h>
#include <string.h>
#include "moor/moor.h"
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    default: {
        ...
        moor_socks5_parse_request(&client, msg, len, &cmd);
        break;
    }
    }
    return 0;
